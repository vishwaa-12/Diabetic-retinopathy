...
class Diagnosis:
    ...
    @staticmethod
    def get_all(sort_by='date', limit=100, skip=0, search_query=None):
        """
        Get diagnoses with pagination and filtering.
        - Default: Shows records from last 30 days that are NOT archived
        - Search: Searches ALL records (including old/archived) by Name or ID
        """
        ...
        if search_query:
            # If searching, look everywhere (ignore archive status/date)
            # Search in patient_info.name, patient_id, or diagnosis_class
            query = {
                '$or': [
                    {'patient_info.name': {'$regex': search_query, '$options': 'i'}},
                    {'patient_id': {'$regex': search_query, '$options': 'i'}},
                    {'diagnosis_class': {'$regex': search_query, '$options': 'i'}}
                ]
            }
        else:
            # Default View: Only recent (last 30 days) AND Not Archived
        ...
        return list(mongo.db.diagnoses.find(query)
                    .sort(sort_by, -1)
                    .skip(skip)
                    .limit(limit))
    ...
    def get_by_id(diagnosis_id):
        ...
        try:
            doc = mongo.db.diagnoses.find_one({'_id': ObjectId(diagnosis_id)})
            return Diagnosis.to_dict(doc)
        except:
            return None

    @staticmethod
    def archive(diagnosis_id):
        """Soft delete: Mark as archived instead of deleting data"""
        from bson.objectid import ObjectId
        mongo.db.diagnoses.update_one(
    ...
    def to_dict(diagnosis_doc):
        ...
        diagnosis_dict = {
            ...
            'patient': {
                ...
                'dob': diagnosis_doc.get('patient_info', {}).get('dob', ''),
